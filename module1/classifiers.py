...
def is_iban_ro(text):
    t = text.strip().upper()
    return bool(re.fullmatch(r'RO[0-9O]{2}(?: ?[A-Z0-9]{4}){5}', t, re.IGNORECASE))
...
def rule_classify(text):
    ...
    if is_cnp(t) or is_iban_ro(text) or is_email(text):
        return "I"
    return None # needs LLM


def llm_classify(texts):
    client = genai.Client(api_key='AIzaSyC9i7oVPxtHnUhBKEhGitJwsjCQRDwMj8k')
...
def classify_texts(texts):
    ...
    print("Rule-based sensitive:", sensitive)
    print("Sending to LLM:", undecided)
