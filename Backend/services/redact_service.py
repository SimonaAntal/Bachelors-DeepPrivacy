...
from exceptions.encrypt_exceptions import EmbeddingException, DecryptionException
from services.pris_service import PRIS


device = 'cuda' if torch.cuda.is_available() else 'cpu'
MODEL_PATH = os.getenv("MODEL_PATH")

pris_model = PRIS(num_blocks=8).to(device)  # image encryption / decryption model
checkpoint = torch.load(MODEL_PATH, map_location=device)
pris_model.load_state_dict(checkpoint)
pris_model.eval()

host_folder = os.getenv("HOST_FOLDER")
...
def pixelate(crop, block_size=None):
    h, w = crop.shape[:2]

    # proportional block size with region dim
    if block_size is None:
        block_size = max(10, int(min(h, w) * 0.1))

    # downscale and upscale => mozaic effect
    small_w = max(1, w // block_size)
    small_h = max(1, h // block_size)

    temp = cv2.resize(crop, (small_w, small_h), interpolation=cv2.INTER_LINEAR)
    return cv2.resize(temp, (w, h), interpolation=cv2.INTER_NEAREST)

def redact_and_embed(img, mask, aes_key = None):
    ...
    with torch.no_grad():
        for (i, x, y, w, h) in regions:
            ...
            container_name = f"{container_id}.tiff"
            save_img(container_np, os.path.join(output_container, container_name))

            # pixelate sensitive region
            region_pixelated = pixelate(crop)
            redacted[y:y + h, x:x + w] = region_pixelated
    ...
    try:
        ...
        raise EmbeddingException("Image does not have enough capacity") from e
...
def recover_from_image(img, key):
    try:
        ...
        raise DecryptionException("Invalid recovery key or corrupted image")
    ...
    with torch.no_grad():
        for r in regions:
            ...
            secret = pris_model.extract(container, True)
